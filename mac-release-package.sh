...
PACKAGE=RDPC-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=RDPC-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} RDPCoin.exe RDPCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe RDPCoin.exe
cp installer/lib/JavaExe.exe RDPCoinservice.exe
cp -a ${FILES} nxt
